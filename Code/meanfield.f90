...
SUBROUTINE skyrme
  ...
  !***********************************************************************
   ...
   !                              on 'spot' as workspace
      ...
      FORALL(icomp=1:3,ix=1:nx,iy=1:ny,iz=1:nz)
         spot(ix,iy,iz,icomp,1) =-b4q*spot(ix,iy,iz,icomp,1) -b4*spot(ix,iy,iz,icomp,2)  
         spot(ix,iy,iz,icomp,2) =-b4q*spot(ix,iy,iz,icomp,2) -b4*spot(ix,iy,iz,icomp,1)   
      END FORALL
     !
   ENDIF
   ...
   IF(toddls) THEN  
      FORALL(is=1:2,ix=1:nx,iy=1:ny,iz=1:nz)
         pout(ix,iy,iz,is) = pout(ix,iy,iz,is)  &
            + CMPLX(spot(ix,iy,iz,1,iq),(-3+2*is)*spot(ix,iy,iz,2,iq),db)*pinn(ix,iy,iz,3-is) &
            +(3-2*is) spot(ix,iy,iz,3,iq)*pinn(ix,iy,iz,is)
      END FORALL
   ENDIF
   !**********************************************************************
   !        J_ij Lesinski way                                            *
