MODULE Forces
  ...
  ! Record defining pairing
     INTEGER :: s2,sf,st,di,la
     REAL(db):: te,toten
     LOGICAL :: s2on        ! switch for S^2 term
     LOGICAL :: lapson      ! switch for lapS term
     LOGICAL :: ston        ! switch for S.T term
     LOGICAL :: j2on        ! switch for J^2 term (with S.T)
     LOGICAL :: jfon        ! switch for J^2 term (with S.F)
     LOGICAL :: sfon        ! switch for S.F term
     LOGICAL :: divson      ! switch for divS term
  TYPE Pairing
...
CONTAINS
  ...
  LOGICAL :: divson     ! switch for divS term
    ...
    NAMELIST /force/ name,pairing, &
         ...
         ipair,v0prot,v0neut,rho0pr,turnoff_zpe,te,toten, &
         s2,sf,st,la,di
    ...
    IF(wflag) THEN
      ...
      WRITE(*,*) 'The Skyrme force invariant quantities ','s2=',s2,'sf=',sf
      WRITE(*,*) 'la=',la,'di=',di
      WRITE(*,*) 'The Tensor force values are','te=',te,'toten=',toten
      WRITE(*,*) 's2on=',s2on,'  sfon=',sfon,'  jfon=',jfon
      WRITE(*,*) 'ston=',ston,'  j2on=',j2on
      WRITE(*,*) 'divson=',divson,'  lapson=',lapson  
