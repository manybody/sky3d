...
MODULE Params
  ...
  COMPLEX(db)        :: cmplxone=CMPLX(1.0d0,0.0d0) !< a complex one.
  COMPLEX(db)        :: cmplxzero=CMPLX(0.0d0,0.0d0)!< a complex zero.
  ...
  CHARACTER(LEN=80) :: diffenergiesfile='diffenergies.res'!< difference in energy for time-dependent
  !!part with absorbing boundaries.
  ...
  CHARACTER(LEN=80) :: tabcfile='tabc.res'               !<TABC averaged output.
  ...
  !>@name TABC control
  !>@{
  INTEGER :: tabc_nprocs=1 !< number of TABC processes.
  INTEGER :: tabc_myid=0   !< rank of current TABC process.
  !>@}
END MODULE Params
