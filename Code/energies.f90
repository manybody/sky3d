    ...
    DO iz=1,nz
      DO iy=1,ny
        DO ix=1,nx
          ...
          rhon=rho(ix,iy,iz,1)
          rhop=rho(ix,iy,iz,2)
          ecorr=ecorr-f%power*wxyz*rhot**f%power*(b3*rhot**2 &
               -b3p*(rhop**2+rhon**2))/6.D0
        ENDDO
      ENDDO
    ENDDO
   ...
   IF(s2on) THEN
    DO iz=1,nz
      DO iy=1,ny
        DO ix=1,nx
          ...
          ecorrs=ecorrs-f%power*wxyz*rhot**f%power*(b2*sdenst**2 &
               -b2p*(sdensp**2+sdensn**2))/6.D0
          ENDDO
        ENDDO
      ENDDO
    ENDDO
    ...
    IF(tcoul) THEN
      ...
      ELSE
        sc=0.D0
      END IF
            ehfc= wxyz*sum((0.5D0*rho(:,:,:,2)*wcoul(:,:,:) &
